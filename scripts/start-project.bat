...
if exist "CLAUDE.md" (
  powershell -Command "^
    ...
    $contextLines = @('', '### 📋 Specifications', 'Xem `docs/SPECIFICATIONS.md` để chi tiết đầy đủ về chức năng, requirements, và design.', '', '### 📋 Task Structure — Dependency-Driven', 'Dùng **Dependency-Driven approach**:', '- `tasks/layer-0-todo.md` — Foundation (no dependency)', '- `tasks/layer-1-todo.md` — Depends on Layer 0 (tạo khi cần)', '- `tasks/layer-2-todo.md` — Depends on Layer 1 (tạo khi cần)', '- ... (thêm layer tùy scope)', '- `tasks/done.md` — Completed tasks', '', '**Quy tắc:**', '- Số layer phụ thuộc vào scope breakdown + dependency analysis', '- Các task trong cùng layer có thể làm song parallel', '- Chỉ khi layer N hoàn toàn xong → mới bắt đầu layer N+1', '- Xem `docs/SCOPE_BREAKDOWN.md` để chi tiết'); ^
    $joinedSection = ($contextLines -join \"`n\") -replace '\\r$', ''; ^
    $content = $content -replace '(## Context)', \\\"`$1$joinedSection\\\"; ^
