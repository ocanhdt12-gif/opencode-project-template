...
## RESPONSIVE CHECKLIST GATE (BẮT BUỘC — dùng trong Phase 5 Review)
...
- [ ] Layout mobile-first: base styles cho mobile, `min-width` media query để enhance
- [ ] Grid dùng `auto-fit`/`minmax` hoặc responsive column classes (không cột cố định)
- [ ] Container không fixed width px; dùng `min()`/`max()`/`%`/`fr`/`vw`
